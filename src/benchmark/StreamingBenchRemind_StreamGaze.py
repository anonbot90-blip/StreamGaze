    ...
    def eval(self, data, model, output_path):
        ...
        for subset in tqdm.tqdm(data, desc="Evaluating Proactive/Remind Tasks"):
            ...
            for question in subset["questions"]:
                ...
                def _process_test(indexed_test):
                    i, test = indexed_test
                    ...
                    # Store response in test_info (mutation is intentional —
                    # matches original serial behavior of in-place updates).
                ...
                indexed = list(enumerate(test_info))
                if _INNER_THREADS > 1 and len(indexed) > 1:
                    with ThreadPoolExecutor(max_workers=_INNER_THREADS) as ex:
                        list(ex.map(_process_test, indexed))
                else:
                    for it in indexed:
                        _process_test(it)
