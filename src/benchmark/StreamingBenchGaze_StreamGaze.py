...
_INNER_THREADS = max(1, int(os.environ.get("STREAMGAZE_INNER_THREADS", "1")))
...
class StreamingBenchGaze_StreamGaze(Benchmark):
    ...
    def eval(self, data, model, output_path):
        ...
        """
        # Resume support: if output_path exists, load prior partial results and
        # skip entries whose predictions are already complete.
        results = []
        done_by_video = {}
        if os.path.exists(output_path):
            try:
                ...
                for e in prior:
                    vp = e.get("video_path", "")
                    preds = e.get("model_predictions", [])
                    qs = e.get("questions", [])
                    if vp and preds and len(preds) == len(qs):
                        done_by_video[vp] = e
                if done_by_video:
                    print(f"🔄 Resume: {len(done_by_video)} entries already complete in {output_path}")
            except Exception as ex:
                print(f"⚠️  Could not parse prior results at {output_path}: {ex}")

        for entry in tqdm(data, desc="Evaluating Present/Future Tasks"):
            video_path = entry.get("video_path", "")
            video_name = os.path.basename(video_path)

            # Resume: skip if this entry is already fully predicted
            if video_path in done_by_video:
                results.append(done_by_video[video_path])
                continue
            ...
            def _process_one(question_data):
            ...
            if _INNER_THREADS > 1 and len(questions) > 1:
                with ThreadPoolExecutor(max_workers=_INNER_THREADS) as ex:
                    preds = list(ex.map(_process_one, questions))
            else:
                preds = [_process_one(q) for q in questions]
            result_entry["model_predictions"] = preds
